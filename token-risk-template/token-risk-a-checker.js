const TOKEN_RISK_APP = (() => {
  ...
  function buildMetricSignals(metrics) {
    ...
    return [
      ...
      `Liquidity: ${compactMoney(metrics.liquidityUsd)}`,

      `Volume 24h: ${compactMoney(metrics.volume24h)}`,

      `Price 24h: ${compactPercent(metrics.priceChange24h)}`,

      `Pair age: ${safeText(metrics.pairAge, "N/A")}`,

      `FDV: ${compactMoney(metrics.fdv)}`
    ...
    return `
      ...
      <div class="result-card ${risk}">
        ...
        <div class="result-top">
          ...
          <div class="risk ${risk}">Risk Level: ${iconForRisk(risk)}</div>
        ...
        <div class="section">
          ...
          <ul class="signal-list">
            ...
            <li class="signal-item">${escapeHtml(`Token: ${tokenName}`)}</li>

            ${metricSignals.map(item => `<li class="signal-item">${escapeHtml(item)}</li>`).join("")}
        ...
        <div class="section">
          ...
          <ul class="signal-list">
            ...
            ${detectedSignals.map(item => `<li class="signal-item">${escapeHtml(item)}</li>`).join("")}
        ...
        <div class="section">
          ...
          <ul class="signal-list">
            ...
            ${positiveSignals.map(item => `<li class="signal-item">${escapeHtml(item)}</li>`).join("")}
  ...
  async function check() {
    ...
    try {
      ...
      if (data?.limit) {
        ...
        if (typeof window.showUpgrade === "function") window.showUpgrade();
  ...
  function init() {
    ...
    if (params.get("subscribed") === "true") {
      ...
      try { localStorage.setItem(BROWSER_SUB_KEY, "true"); } catch (_) {}

      if (postPurchase) postPurchase.style.display = "block";
