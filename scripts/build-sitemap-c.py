...
DISCOVERY_URLS = [
    f"{BASE_URL}/discovery-c/latest/",
    f"{BASE_URL}/discovery-c/today/",
    f"{BASE_URL}/discovery-c/all/",
    f"{BASE_URL}/discovery-c/hub/",
    f"{BASE_URL}/discovery-c/random/",
]
...
def write_page_sitemap():
    today = get_today()
    page_urls = get_all_html_urls(CHECK_DIR, "check")
    ...
    print(f"✅ Built {SITEMAP_PAGE_FILE} with {len(page_urls)} URLs")
...
def write_discovery_sitemap():
    ...
    print(f"✅ Built {SITEMAP_DISCOVERY_FILE} with {len(DISCOVERY_URLS)} URLs")
