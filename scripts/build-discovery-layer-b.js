...
const CHECK_DIR = path.join(__dirname, "../scam-check-now-b");
const OUTPUT_BASE = path.join(__dirname, "../discovery-b");
...
function toUrl(file) {
  ...
  return BASE_URL + "/scam-check-now-b" + relative;
}
...
function formatLinkLabel(url) {
  return url
    .replace(BASE_URL + "/scam-check-now-b/", "")
    .replace(/\//g, " / ")
    .replace(/-/g, " ")
    .replace(/\s+/g, " ")
    .trim();
...
function getPageMeta(title) {
  if (title === "Latest Scam Checks") {
    return {
      badge: "Updated continuously",
      kicker: "Latest pages",
      eyebrow: "Fresh scam checks",
      intro:
        "Browse the newest scam check pages and recently updated warning topics. Use these pages when you want the latest suspicious messages, link checks, and account warning patterns in one place.",
      statA: "Latest additions",
      statB: "Fast navigation",
      statC: "Built for reuse"
    };
  }

  if (title === "Today's Scam Alerts") {
    return {
      badge: "Fresh today",
      kicker: "Today’s scam alerts",
      eyebrow: "New pages added today",
      intro:
        "See the freshest scam alert pages added today. This page helps people move quickly when a suspicious message, delivery alert, payment warning, or login request shows up right now.",
      statA: "Today’s updates",
      statB: "Quick scan",
      statC: "High urgency topics"
    };
  }

  if (title === "All Scam Checks") {
    return {
      badge: "Full directory",
      kicker: "All scam checks",
      eyebrow: "Browse the full library",
      intro:
        "Browse the wider scam check library in one place. Use this directory to explore suspicious message pages across delivery scams, payment alerts, job scams, crypto traps, and account warning topics.",
  ...
  if (title === "Random Scam Checks") {
    return {
      badge: "Discovery mode",
      kicker: "Random scam checks",
      eyebrow: "Explore different scam patterns",
      intro:
        "Explore a rotating mix of scam check pages. This is useful for discovering adjacent scam patterns, similar message types, and warning pages people may want to revisit or share later.",
      statA: "Mixed topics",
      statB: "Discovery paths",
      statC: "Reuse friendly"
    };
  }

  return {
    badge: "Central navigation",
    kicker: "Scam detection hub",
    eyebrow: "Start from the main hub",
    intro:
      "Use this hub to move through scam detection topics quickly. It brings together high-value scam check pages so people can browse, compare patterns, and return when the next suspicious message appears.",
    statA: "Central access",
    statB: "Clear structure",
    statC: "Return visits"
  };
}
...
<link rel="canonical" href="${BASE_URL}/discovery-b/${title === "Latest Scam Checks" ? "latest" : title === "Today's Scam Alerts" ? "today" : title === "All Scam Checks" ? "all" : title === "Random Scam Checks" ? "random" : "hub"}/">

<meta property="og:title" content="${escapeHtml(title)}">
<meta property="og:description" content="${escapeHtml(description)}">
<meta property="og:type" content="website">
<meta property="og:url" content="${BASE_URL}/discovery-b/${title === "Latest Scam Checks" ? "latest" : title === "Today's Scam Alerts" ? "today" : title === "All Scam Checks" ? "all" : title === "Random Scam Checks" ? "random" : "hub"}/">
...
<div class="top-bar">
  <a class="logo" href="${BASE_URL}/scam-check-now-b/">
  ...
  <div class="top-actions">
    ...
    <a class="upgrade-top" href="${BASE_URL}/scam-check-now-b/">Run Check</a>
  </div>
</div>
...
<div class="page-shell">
  ...
  <div class="hero">
    <div class="hero-badge-row">
      <div class="hero-badge">${escapeHtml(meta.badge)}</div>
      <div class="hero-badge">Premium warning pages</div>
  ...
  <div class="container">
    ...
    <div class="links-card">
      ...
      <ul class="links-list">
        ${links.map(l => `
        <li>
          <a class="link-item" href="${escapeHtml(l)}">
            <span class="link-copy">
              <span class="link-title">${escapeHtml(formatLinkLabel(l))}</span>
              <span class="link-sub">Open this scam check page and review warning signs, risk patterns, and what to do next.</span>
            </span>
  ...
  <footer class="footer">
    <div>
      ...
      <a href="${BASE_URL}/website-policies/scam-check/" target="_blank" rel="noopener noreferrer">Terms and Privacy Policy</a>.
    </div>
...
function run() {
  ...
  console.log("✅ B discovery layer built");
}
