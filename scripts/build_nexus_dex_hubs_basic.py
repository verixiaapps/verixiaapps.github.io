...
# Protect hubs + enforce valid child pages
...
def main():
    ...
    for hub_slug, terms in CLUSTERS.items():
        ...
        title = f"{title_case(hub_slug.replace('-', ' '))}: Features, Workflows & Related Pages"

        description = trim_meta_description(
            "Explore Nexus DEX features, workflows, and related pages for self-custodial trading from your Solana wallet."
        )
